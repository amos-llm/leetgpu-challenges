...
__global__ void fp8_per_channel_quantize_kernel(const float* x, uint8_t* y, float* scales, int M,
                                                int K) {
    ...
    for (int token_idx = threadIdx.x; token_idx < M; token_idx += blockDim.x) {
        ...
        y[token_idx * K + channel_idx] = __nv_fp8_e4m3(val / scale).__x;
    }
...
extern "C" void solve(const float* x, uint8_t* y, float* scales, int M, int K) {
    ...
    fp8_per_channel_quantize_kernel<kBlockSize><<<K, kBlockSize>>>(x, y, scales, M, K);
