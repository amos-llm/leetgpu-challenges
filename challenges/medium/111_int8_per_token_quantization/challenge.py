...
class Challenge(ChallengeBase):
    name = "INT8 Per-Token Quantization"
    atol = 0.01
    rtol = 0.01
    num_gpus = 1
    access_tier = "free"
    ...
    def generate_functional_test(self) -> List[Dict[str, Any]]:
        ...
        # Edge: single column (each element is its own row)
        ...
        # All zeros: tests scale=1.0 fallback
        x4 = torch.zeros(8, 8, device=device, dtype=dtype)
        y4 = torch.empty(8, 8, device=device, dtype=torch.int8)
        ...
        x6 = torch.empty(32, 64, device=device, dtype=dtype).uniform_(-10.0, 10.0)
        y6 = torch.empty(32, 64, device=device, dtype=torch.int8)
        s6 = torch.empty(32, device=device, dtype=dtype)
        tests.append({"x": x6, "y": y6, "scales": s6, "M": 32, "K": 64})

        # Non-power-of-2 dimensions
        torch.manual_seed(99)
        x7 = torch.empty(30, 50, device=device, dtype=dtype).uniform_(-100.0, 100.0)
        y7 = torch.empty(30, 50, device=device, dtype=torch.int8)
        s7 = torch.empty(30, device=device, dtype=dtype)
        tests.append({"x": x7, "y": y7, "scales": s7, "M": 30, "K": 50})

        # Rows with very different magnitudes
        x8 = torch.tensor(
            [[0.01, 0.02], [100.0, -50.0], [0.0, 0.0]],
            device=device,
            dtype=dtype,
        )
        y8 = torch.empty(3, 2, device=device, dtype=torch.int8)
        s8 = torch.empty(3, device=device, dtype=dtype)
        tests.append({"x": x8, "y": y8, "scales": s8, "M": 3, "K": 2})

        # Realistic LLM activation: batch=32, hidden=4096
        torch.manual_seed(7)
        x9 = torch.empty(32, 1024, device=device, dtype=dtype).normal_(0.0, 0.5)
        y9 = torch.empty(32, 1024, device=device, dtype=torch.int8)
