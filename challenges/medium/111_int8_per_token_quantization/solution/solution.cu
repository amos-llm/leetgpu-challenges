...
__global__ void int8_per_token_quantize_vectorized_kernel(const float* x, int8_t* y, float* scales,
                                                          int K) {
    ...
    for (int vec_idx = threadIdx.x; vec_idx < num_vecs; vec_idx += blockDim.x) {
        float4 x_vec = reinterpret_cast<const float4*>(x_base)[vec_idx];
        char4 y_vec;
        y_vec.x = __float2int_rn(cuda::std::clamp(x_vec.x / scale, -128.0f, 127.0f));
        y_vec.y = __float2int_rn(cuda::std::clamp(x_vec.y / scale, -128.0f, 127.0f));
        y_vec.z = __float2int_rn(cuda::std::clamp(x_vec.z / scale, -128.0f, 127.0f));
        y_vec.w = __float2int_rn(cuda::std::clamp(x_vec.w / scale, -128.0f, 127.0f));
        reinterpret_cast<char4*>(y_base)[vec_idx] = y_vec;
    }
...
template <int kBlockSize>
__global__ void int8_per_token_quantize_scalar_kernel(const float* x, int8_t* y, float* scales,
                                                      int K) {
    using BlockReduce = cub::BlockReduce<float, kBlockSize>;
    __shared__ typename BlockReduce::TempStorage temp_storage;
    __shared__ float global_max_abs;
    ...
    for (int i = threadIdx.x; i < K; i += blockDim.x) {
        float val = x_base[i];
        y_base[i] = __float2int_rn(cuda::std::clamp(val / scale, -128.0f, 127.0f));
    }
...
extern "C" void solve(const float* x, int8_t* y, float* scales, int M, int K) {
    ...
    if (K % 4 == 0 && x_aligned && y_aligned) {
        int8_per_token_quantize_vectorized_kernel<kBlockSize><<<M, kBlockSize>>>(x, y, scales, K);
    } else {
        int8_per_token_quantize_scalar_kernel<kBlockSize><<<M, kBlockSize>>>(x, y, scales, K);
    }
