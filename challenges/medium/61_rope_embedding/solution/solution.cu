...
__global__ void rope_kernel(float* Q, float* cos, float* sin, float* output, int M, int D) {
    ...
    float q1_items[kItemsPerThread];
    float q2_items[kItemsPerThread];
    float c1_items[kItemsPerThread];
    float c2_items[kItemsPerThread];
    float s1_items[kItemsPerThread];
    float s2_items[kItemsPerThread];
    int block_offset = blockIdx.y * kBlockSize * kItemsPerThread;
