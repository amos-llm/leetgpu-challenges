...
def attn_kernel(
    ...
    for n in range(0, N, BLOCK_SIZE_N):
        ...
        qk *= tl.rsqrt(tl.cast(d, tl.float32))

        masked_qk = tl.where(
            (offs_m[:, None] >= offs_n[None, :]) & (offs_n[None, :] < N), qk, float("-inf")
        )

        row_max_new = tl.maximum(row_max, tl.max(masked_qk, axis=1, keep_dims=True))
        alpha = tl.exp(row_max - row_max_new)

        p = tl.exp(masked_qk - row_max_new)
        v = tl.load(
